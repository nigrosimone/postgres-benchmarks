...
summary(() => {
  ...
  group("unprepared", () => {
    bench("brianc/node-postgres (pg-native)", () =>
      pgNative.query({ text: `select 1 as x` })
    );

    if (global.gc) global.gc();

    bench("brianc/node-postgres (pg)", () =>
      pgVanilla.query({ text: `select 1 as x` })
    );

    if (global.gc) global.gc();

    bench("porsager/postgres (postgres)", () => sqlUnprepared`select 1 as x`);

    if (global.gc) global.gc();

    bench("porsager/postgres (unsafe)", () =>
      sql.unsafe(`select 1 as x`)
    );
